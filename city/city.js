...
function onSubmit() {
  ...
  if (city === "" && allErrorMessages.length === 0) {
    inputDiv.appendChild(errorMessage);
  } else if (city != "" && allErrorMessages.length === 1) {
  ...
  //console.log('error length is', allErrorMessages.length);
}

function addElements(data, city) {
...
function makeApiCall(city) { //clicks
  //makes the api link
  const openWeatherApiCall = `http://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${openWeatherApiKey}&units=imperial`;

  //makes the api call to retrieve the json link
...
function addGraph(city) {
  ...
  for (i = 7; i >= 0; i--) {
    ...
    let weatherApiCall = `http://api.weatherapi.com/v1/history.json?key=${weatherApiKey}&q=${city}&dt=${pastFullDay}&debug=1`;
